...
**Certificate previews (web CV):** Public Microsoft/BilgeAdam certs and diploma expand inline on `/tr/cv` with a WebP preview + PDF download. Source PDFs live in `content/resume/sources/`; run `pnpm credentials:generate` after adding or replacing a document to copy PDFs into `apps/web/public/credentials/` and regenerate preview images (commit both).
